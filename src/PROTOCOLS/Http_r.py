...
def http_analysis(sequence):
    ...
    request_line_format = ["Method", "URL", "Version"]
    request_line_value = L_headers[0].split(chr(int('20', 16)), 2)

    return [("Request line", [L_headers[0]] + [(request_line_format[i], request_line_value[i]) for i in range(3)])] + [("Header line {}".format(x+1), L_headers[1:][x]) for x in range(len(L_headers[1:]))]
